...
# Final sums
n_OTUs_library_filtered_control <- nrow(OTUs_library_filtered_samples) %>% print(.)
n_reads_library_filtered_control <- sum(OTUs_library_filtered_samples %>% select(-OTU_ID)) %>% print(.)
n_samples_library_filtered_control <- (ncol(OTUs_library_filtered_samples) - 1) %>% print(.)
...
# Read the FASTA file
fasta_sequences <- readDNAStringSet("../data/OTUs/OTUs.fasta")
...
writeXStringSet(filtered_fasta, "../data/OTUs/OTUs_abundance_filtered.fasta")
...
OTUs_library_filtered_samples %>%
  ...
  fwrite(
    .,
    "../data/OTUs/OTUs_abundance_filtered.txt",
    sep = "\t"
)

# # Organise taxonomy
# fread("../data/taxonomy/taxonomy.txt") %>%
#   mutate(
...
#   fwrite(
#     .,
#     "../data/taxonomy/taxonomy_abundance_filtered.txt",
#     sep = "\t"
# )
