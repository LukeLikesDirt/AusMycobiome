...
dir.create(file.path(path, 'denoised'))
dir.create(file.path(path, 'chimera_filtered'))
...
for (run in 1:num_runs) {
  ...
  for (sam in sample.names) {
    ...
    dds[[sam]] <- dada(
      derep, err = err, multithread = TRUE, DETECT_SINGLETONS = TRUE
      )
  }
...
for (run in 1:num_runs) {
  seqtab_file <- file.path(path, paste0('denoised/', run, '_seqtab.rds'))
