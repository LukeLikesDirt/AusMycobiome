...
source("get_cutoffs.R")
source("taxa_match.R")

# Retrieve command-line arguments
args <- commandArgs(trailingOnly = TRUE)
taxa_file_path <- args[1]
cutoff_file_path <- args[2]
asv_sequences_path <- args[3]
asv_table_path <- args[4]
otu_output_path <- args[5]
threads <- as.integer(args[6])
minlen <- as.integer(args[7])
